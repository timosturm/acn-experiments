...
from src.imitation.objective import objective_IL, objective_RL
from src.rewards import sparse_soc_reward
from src.utils import AV_pod_ids, create_df, create_study_with_retries, get_generator, get_power_function, get_steps_per_epoch
from src.cleanRL.environment import make_env
...
validation_generator.seed = 8734956
_ = validation_generator.reset()

steps_per_epoch_val = ic(
    get_steps_per_epoch(validation_generator)
)
...
study_name: str = "gauss_charging_centered_AV_IL"
hiddens = [2048, 512, 128]

with tempfile.TemporaryDirectory() as td:
    training_f = os.path.join(td, 'training')
    validation_f = os.path.join(td, 'validation')

    with open(training_f, "wb") as tf:
        with open(validation_f, "wb") as vf:
            model = ACNSchedule(SortedSchedulingAlgo(first_come_first_served))
            create_df(model, tf, make_env(
                train_config, 0.99, 0)(), steps_per_epoch)
            create_df(model, vf, make_env(
                validation_config, 0.99, 0)(), steps_per_epoch_val)

    args = MyArgs(
        exp_name="IL",
        wandb_project_name="il",
        wandb_group=study_name,
        seed=42,  # TODO
        imitation=ImitationArgs(
            train_ds=training_f,
            validation_ds=validation_f,
            agent_class=Agent,
            hiddens=hiddens,
        ),
        eval=EvalArgs(
    ...
    if __name__ == "__main__":
        study = create_study_with_retries(study_name, direction="minimize")
