...
def ranking_schedule_plus() -> SimActionFactory:
    ...
    def to_schedule(
        ...
        def sort_fn(sessions: List[SessionInfo], iface: Interface) -> List[SessionInfo]:
            ...
            ranking_dict = {station_id: ranking for station_id, ranking in zip(station_ids, action)}

            out = sorted(sessions, key=lambda session: ranking_dict[session.station_id])
        ...
        scheduler.register_interface(iface)
        sessions = list(np.array(iface.active_sessions())[action > 0])
        
        action_out = {station: [0] for station in iface.station_ids} | scheduler.schedule(sessions)
